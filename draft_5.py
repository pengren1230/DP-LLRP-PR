# -*- coding: utf-8 -*-
"""
Created on Tue Jun 10 13:27:27 2025
...
initial_state = {
    "served_mask": 0,
    "vehicle_counts": [vehicles_per_depot] * num_depots,
    "latency": 0,
    "routes": [],
}
    ...
    for depot_idx, depot_id in enumerate(depot_ids):
        ...
        for r in range(1, min(4, len(unserved)) + 1):
            for route in itertools.combinations(unserved, r):
                ...
                new_vehicle_counts = vehicle_counts[:]
                new_vehicle_counts[depot_idx] -= 1
...
# Rollout algorithm
def rollout_dp(max_depth=3, timeout_seconds=180):
    pq = []
    tie_counter = count()
    ...
    while pq:
        ...
        if len(state["routes"]) >= max_depth * num_customers:
            continue
...
# Run it
#best_result = rollout_dp(max_depth=3)
best_result = rollout_dp(max_depth=3, timeout_seconds=360)

if best_result is not None:
    routes_df = pd.DataFrame(best_result["routes"])
    print("✅ Best approximate solution found:")
    print("Total latency:", best_result["latency"])
    print(routes_df)
else:
    print("❌ No solution found within the time limit.")

routes_df = pd.DataFrame(best_result["routes"])
routes_df["Vehicle_ID"] = routes_df.groupby("depot").cumcount() + 1
