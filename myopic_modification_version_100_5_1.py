...
class DeepSet(nn.Module):
    ...
    def forward(self, depot_vec, node_feats):
        outs = []
        for X in node_feats:
            s = self.phi(X).sum(dim=0) if X.numel() > 0 else torch.zeros(64)
...
def latency_fast_cached(d_idx, subset_tuple):
    ...
    if _surrogate is not None:
        ...
        with torch.no_grad():
            y = float(_surrogate(dep, [X]).squeeze())
        return y
    ...
    for t in range(time_horizon):
        ...
        for dname in depot_names:
            ...
            for subset_names in cands:
                ...
                tail = estimate_tail_value(remaining, inv_left, r_left, v_left, t+1)
        ...
        if best is None:
            for dname in depot_names:
                ...
                if nearest is not None:
                    ...
                    best = (lat, dname, [nearest], demand_table[t][nearest], lat)
                    break
...
if __name__ == "__main__":
    print(f"Surrogate (PyTorch) enabled: {bool('torch' in globals() and TORCH_OK and (_surrogate is not None))}")
